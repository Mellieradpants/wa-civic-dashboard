...
// This is real-example discovery for the two-independent-instructions
// detection design — not a pass/fail check.
...
for (const billNumber of batch) {
  ...
  for (const section of data.sections || []) {
    ...
    for (const sentence of splitSentences(section.text)) {
      ...
      candidateCount++;
      console.log(`--- CANDIDATE ${candidateCount} — bill ${billNumber}, section ${section.id}, modals: ${modalMatches.join(", ")} ---`);
