...
- `parseActorActionCondition` (L5 AAC) returns an **array** of one or two instruction groups, not always exactly one. A sentence with two independent, equally-weighted instructions joined by "and" — different actor, different requirement each, e.g. "The matter must be heard de novo and the court must include written findings" — produces two full ISC units instead of one unit with the second instruction's actor and modal silently swallowed into the first unit's action text. `runPipeline`'s sentence→unit mapping is a `flatMap`, not a `map`+`filter`, because of this; `sourceLocation` is assigned after flattening (a running count across the section's units, not each sentence's own index) for the same reason. Detection only fires on a genuinely distinct second actor: a modal sitting immediately after "and" with nothing between is the already-handled same-actor case (`renderer.js`'s `splitCompoundAction`) and is deliberately left alone; a back-reference whose own last word is a bare pronoun (`it/they/he/she/this/these`) is treated as pointing back to the first actor, not a new one — but a non-pronoun back-reference ("such board", "the college board") is not rejected just because it's a back-reference, only a bare-pronoun one is. Identical wording between the two actors is never on its own a reason to reject — two teams both named "one of the teams" serving different cities are still two real, distinct instructions. Only catches exactly two instructions (a third "and"-joined clause is not detected) and only "and"-joined pairs (not semicolon-joined). The second instruction's own `conditions`/`deadlines`/`jurisdiction`/`mechanism`/`enforcement`/`risk` are left empty rather than duplicating the first instruction's onto it, since L6-L9 still run once on the whole sentence text and can't be reliably attributed to one clause or the other — the same choice `renderer.js`'s `clause2Fields` already makes for the same-actor compound-split case. The second-instruction scan has its own "May [date]" guard (a bare "may" immediately followed by a day-number doesn't count as a modal) — this guard is scoped to the second-instruction detector only; the primary modal match's own susceptibility to the same "May [date]" collision (see "Known pipeline limitations" below) is unchanged.
