...
- **Action slot omission** — the four colon-frame lenses (`modality_shift`, `scope_change`, `threshold_shift` non-cash, `action_domain_shift`) pass `{ action: "" }` to `fillTemplate` and strip any trailing `: ` from the result. No English action phrase is appended. Output is a complete target-language sentence. Do not reintroduce `if (act) raw += ": " + act` — hybrid output that appears translated but contains English action phrases is dishonest and violates the project's core constraint of traceable, deterministic output.
