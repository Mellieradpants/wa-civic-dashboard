...
function translateActionPhrase(verb, obj, lang) {
  ...
  if (obj) {
    ...
    if (fullTrans) {
      return { translated: lcFirst(fullTrans), verbMissing: false, objMissing: false };
    }
  }
  ...
  for (const raw of items) {
    ...
    if (!trans) {
      ...
      if (vTrans) {
        ...
        const frame = DICTIONARY.syntax_frames?.[lang] || ["verb", "object"];
        const oOut = oTrans ? lcFirst(oTrans) : (o ?? null);
        trans = oOut
...
function renderLocalizedSentence(lens, modal, fields, lang) {
  ...
  if (rawAction && lang) {
    ...
    // Double-negation guard: the prohibition templates (cannot/prohibits) already
    // inject "no"/"không"/"не" etc. If the translated action phrase also starts
    // with that language's negation marker, strip it before template fill so the
    // negation doesn't stack. Covers verbs like "unauthorized" → "no autorizado".
    if (modal === "cannot" && action) {
...
export function renderUnit(unit, lang) {
  ...
  if (lang === "so" && sentence) {
    sentence = finalize(sanitizeSomaliOutput(sentence));
  }
