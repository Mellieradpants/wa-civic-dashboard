...
function renderLocalized(lens, lang, fields) {
  ...
  if (lens === "repeal") {
    ...
    const tmpl = sub && act
      ? t.scope_change?.[modal]?.[lang]
      : t.scope_change?.no_actor?.[lang];
    if (tmpl) {
      raw = fillTemplate(tmpl, { actor: sub || "This requirement", action: act || "" });
      if (cond && !alreadyPresent(raw, cond)) raw += `, ${cond}`;
    ...
    if (hasRounding && hasCentAmount) {
      ...
      const tmpl = sub
        ? t.threshold_shift?.[modal]?.[lang]
        : t.threshold_shift?.no_actor?.[lang];
      if (tmpl) {
        raw = fillTemplate(tmpl, { actor: sub || "", action: act || "" });
        if (threshold && !alreadyPresent(raw, threshold)) raw += `, ${threshold}`;
      }
    }
  } else if (lens === "action_domain_shift") {
    const tmpl = sub
      ? t.action_domain_shift?.[modal]?.[lang]
      : t.action_domain_shift?.no_actor?.[lang];
    if (tmpl) {
      raw = fillTemplate(tmpl, { actor: sub || "", action: act || "" });
      if (cond && !alreadyPresent(raw, cond)) raw += `, ${cond}`;
      if (dl && !alreadyPresent(raw, dl)) raw += `, ${dl}`;
    ...
    // modality_shift (default)
    let tmpl;
    if (sub) {
      tmpl = t.modality_shift?.[modal]?.[lang];
    } else {
      const verb = modal === "must" ? "requires" : modal === "may" ? "allows" : "prohibits";
      tmpl = t.modality_shift?.[verb]?.[lang];
    }
    if (tmpl) {
      raw = fillTemplate(tmpl, { actor: sub || "", action: act || "follow this requirement" });
      if (cond && !alreadyPresent(raw, cond)) raw += `, ${cond}`;
      if (dl && !alreadyPresent(raw, dl)) raw += `, ${dl}`;
      if (enforcement) {
