...
// ─── Localized frame ──────────────────────────────────────────────────────────
// Returns the target-language posture label appended to English sentences.
// Format (when actor present): "Qaybtan waxay u baahdaa (The office of financial management)"
// The caller appends this after the English sentence with " — " separator.

function fillTemplate(tmpl, vars) {
  return String(tmpl).replace(/\{(\w+)\}/g, (_, k) => vars[k] ?? "");
}

function getLocalizedFrame(modal, lang, actor) {
  if (!lang || lang === "en") return null;
  const verb = modal === "must" ? "requires" : modal === "may" ? "allows" : "prohibits";
  const tmpl = TRANSLATIONS.modality_shift?.[verb]?.[lang];
  if (!tmpl) return null;
  let frame = fillTemplate(tmpl, { action: "" }).replace(/:\s*$/, "").trim();
  const sub = actor ? cleanActor(actor) : null;
  if (sub && sub.length <= 60) frame += ` (${sub})`;
  return frame;
}
...
export function renderUnit(unit, lang) {
  ...
  if (sectionType.type === "repeal") {
    ...
    if (lang && lang !== "en" && sentence) {
      const frame = getLocalizedFrame(modal, lang, fields.actor);
      if (frame) sentence = `${sentence} — ${frame}.`;
    }
  }
