...
function renderLocalized(lens, lang, fields) {
  if (!lang || lang === "en") return null;
  const t = TRANSLATIONS;
  const { actor, modal, action, conditions, deadlines, enforcement } = fields;
  const act = cleanAction(action);
  const sub = actor ? cleanActor(actor) : null;
  const cond = firstCondition(conditions);
  const dl = firstDeadline(deadlines);
  ...
  if (lens === "repeal") {
    const tmpl = t.repeal?.[lang];
    if (tmpl) raw = fillTemplate(tmpl, { actor: sub || "This section" });
  } else if (lens === "obligation_removal") {
    const tmpl = act
      ? t.obligation_removal?.with_action?.[lang]
      : t.obligation_removal?.no_action?.[lang];
    if (tmpl) raw = fillTemplate(tmpl, { actor: sub || "This section", action: act || "" });
  } else if (lens === "actor_power_shift") {
    const rawAct = cleanAction(action)
      ?.replace(/^be\s+responsible\s+for\s+/i, "")
      .replace(/^responsible\s+for\s+/i, "")
      .trim() || null;
    if (!sub) return null;
    const tmpl = rawAct
      ? t.actor_power_shift?.with_action?.[lang]
      : t.actor_power_shift?.no_action?.[lang];
    if (tmpl) {
      raw = fillTemplate(tmpl, { actor: sub, action: rawAct || "" });
      if (cond && !alreadyPresent(raw, cond)) raw += `, ${cond}`;
    }
  } else if (lens === "scope_change") {
    // Always use no_actor template — avoids English subject before target-language modal verb.
    const tmpl = t.scope_change?.no_actor?.[lang];
    if (tmpl) {
      raw = fillTemplate(tmpl, {});
      if (cond && !alreadyPresent(raw, cond)) raw += `, ${cond}`;
      if (dl && !alreadyPresent(raw, dl)) raw += `, ${dl}`;
    }
  } else if (lens === "threshold_shift") {
    const hasRounding = /\b(round(?:ed|ing)?)\b/i.test(act || "");
    const hasCentAmount = /\bcents?\b|\bnickel\b|\bdime\b|\bfive.cent\b/i.test(act || "");
    if (hasRounding && hasCentAmount) {
      const tmpl = t.threshold_shift_cash?.[lang];
      if (tmpl) raw = fillTemplate(tmpl, { condition: cond || "When you pay cash", action: act || "" });
    } else {
      // Always use no_actor template — avoids English subject before target-language modal verb.
      const threshold = dl || (conditions || []).find((c) => /\d/.test(c)) || cond;
      const tmpl = t.threshold_shift?.no_actor?.[lang];
      if (tmpl) {
        raw = fillTemplate(tmpl, { action: act || "" });
        if (threshold && !alreadyPresent(raw, threshold)) raw += `, ${threshold}`;
      }
    }
  } else if (lens === "action_domain_shift") {
    // Always use no_actor template — avoids English subject before target-language modal verb.
    const tmpl = t.action_domain_shift?.no_actor?.[lang];
    if (tmpl) {
      raw = fillTemplate(tmpl, { action: act || "" });
      if (cond && !alreadyPresent(raw, cond)) raw += `, ${cond}`;
      if (dl && !alreadyPresent(raw, dl)) raw += `, ${dl}`;
    ...
    // modality_shift (default) — use section-level template (requires/allows/prohibits form).
    // Avoids English subject before target-language modal: "{actor} debe {action}" produces
    // a broken sentence when actor and action are English phrases.
    const verb = modal === "must" ? "requires" : modal === "may" ? "allows" : "prohibits";
    const tmpl = t.modality_shift?.[verb]?.[lang];
    if (tmpl) {
      raw = fillTemplate(tmpl, { action: act || "follow this requirement" });
      if (cond && !alreadyPresent(raw, cond)) raw += `, ${cond}`;
      if (dl && !alreadyPresent(raw, dl)) raw += `, ${dl}`;
      if (enforcement) {
        const enfTmpl = t.enforcement_label?.[lang];
        if (enfTmpl) raw += `. ${fillTemplate(enfTmpl, { enforcement })}`;
      }
    }
  }

  return raw ? finalize(raw) : null;
}

// ─── Render one ISC unit ──────────────────────────────────────────────────────

export function renderUnit(unit, lang, { noEnglishFallback = false } = {}) {
  ...
  if (sectionType.type === "repeal") {
    lens = "repeal";
    if (lang && lang !== "en") {
      sentence = renderLocalized("repeal", lang, fields);
    }
    if (!sentence) {
    ...
    if (lang && lang !== "en") {
      sentence = renderLocalized(lens, lang, fields);
    }
    if (!sentence && !noEnglishFallback) {
...
export function renderISC(iscOutput, options = {}) {
  ...
  const rendered = units.map((u) => renderUnit(u, lang)).filter((r) => r.sentence);
...
// ─── Localized render (no English template fallback) ─────────────────────────
// Renders ISC units in the target language. When no localized template exists
// for a given lens the sentence is omitted rather than falling back to English,
// ensuring translation output never contains English structural words.

export function renderISCLocalized(iscOutput, lang) {
  const units = Array.isArray(iscOutput) ? iscOutput : iscOutput?.units || [];
  const rendered = units
    .map((u) => renderUnit(u, lang, { noEnglishFallback: true }))
    .filter((r) => r?.sentence);

  const st = units.find((u) => u.sectionType?.type && u.sectionType.type !== "standard")?.sectionType
    || { type: "standard" };

  const tp = TRANSLATIONS.section_type_prefixes;
  const prefixMap = {
    addition:      tp?.addition?.[lang]      || "New law — ",
    amendment:     tp?.amendment?.[lang]     || "Amends existing law — ",
    appropriation: tp?.appropriation?.[lang] || "Funding — ",
  ...
  const repealFallback = TRANSLATIONS.repeal?.[lang]
  ...
  return {
    plainMeaning: body ? (prefix ? `${prefix}${body}` : body) : "",
    sentences: rendered,
    sectionType: st.type,
    hasContent: rendered.length > 0 || st.type === "repeal",
  };
