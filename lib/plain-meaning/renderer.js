...
export function traceRenderUnit(unit) {
  ...
  // The L3 CFS record is a child of the sentence record, not an ancestor —
  // it's this unit's terminal step, so it's appended after the walk.
  const cfsRecord = records.find(
    (r) => r.producedBy === "L3 CFS" && r.parentNodeId === sentenceRecord.id
  );
  const steps = cfsRecord ? [...ancestry, cfsRecord] : ancestry;
