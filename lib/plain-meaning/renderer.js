...
// {actor} and {action} from parse fields. Actor/action presence determines
// which sub-key to use. Null fields are flagged as [?] — never silently dropped.

function renderLocalizedSentence(lens, modal, fields, lang) {
  const actor = fields.actor ? cleanActor(fields.actor) : null;
  const action = cleanAction(fields.action);
  ...
  return finalize(fillTemplate(tmpl, {
    actor: actor ?? "[?]",
    action: action ?? "[?]",
  }));
}
...
export function renderUnit(unit, lang) {
  ...
  if (sectionType.type === "repeal") {
    ...
    if (lang && lang !== "en") {
      sentence = renderLocalizedSentence(lens, modal, fields, lang);
    } else {
