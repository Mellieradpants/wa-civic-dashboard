...
function translateVerbList(primaryVerbTrans, obj, lang) {
  ...
  for (const raw of items) {
    ...
    if (lang === "so" && trans && translated.length > 0 && !trans.startsWith("iyo ")) {
      trans = "iyo " + trans;
...
function sanitizeSomaliOutput(s) {
  return s
    ...
    // Fallback: fused comma before period on any -aas word (no word boundary required)
    .replace(/(\w+aas),\./g, "$1.")
...
export function renderUnit(unit, lang) {
  ...
  if (lang === "so" && sentence) {
    ...
    sentence = sentence.split("\n\n").map(s => finalize(sanitizeSomaliOutput(s))).join("\n\n");
  }
