...
function translateVerbList(primaryVerbTrans, obj, lang) {
  ...
  for (const raw of items) {
    ...
    // preserves the "and" relationship. Check before English "and" was stripped.
    if (conjMatch && trans) {
      if (lang === "so") {
        trans = "iyo " + trans;
      } else if (lang === "ko") {
...
function sanitizeSomaliOutput(s) {
  return s
    ...
    // Strip comma variants (ASCII, fullwidth, presentation forms) before the period
    .replace(/[,，︐︑]\s*\./g, ".")
    // Strip comma directly attached to a word ending in a Somali definite article suffix
    // before the sentence period: "agaasimayaashaas,." → "agaasimayaashaas."
...
function renderLocalizedSentence(lens, modal, fields, lang) {
  ...
  if (rawAction && lang) {
    ...
    if (modal === "cannot") {
      if (result.inherentNeg) {
        ...
        // inherent_negation guard did not fire — record extracted phrase for
        // inline display so the dashboard reveals what the lookup received.
        debugExtract = `${verb}${obj ? " " + obj : ""}`;
      }
    }
  }
  ...
  let sentence = finalize(fillTemplate(tmpl, {
    actor: actor ?? "[?]",
    action: action ?? "[?]",
  }));
  if (debugExtract && sentence) {
    sentence = sentence.replace(/\.$/, "") + ` [extract: ${debugExtract}].`;
  }
  return { sentence, missingTokens };
}
