...
function buildInclusionNotes(inclusionLists) {
  ...
  return inclusionLists
    .filter((l) => l && l.listText)
    .map((l) => {
      const label = l.classification === "REVERSED" || l.marker === "excluding"
        ? "Does not include"
        : "Named examples";
      // listText starts with the marker word itself (and, for COLON_SUBLIST,
      // its colon) — strip that lead-in so the note doesn't read "Named
      // examples: including X".
      const body = l.listText.trim().replace(/^(including|excluding)\s*:?\s*/i, "");
      return `${label}: ${body}`;
    });
}
...
export function renderUnit(unit, { debug = false } = {}) {
  ...
  // Attach each detected including/excluding list as a plain-English note on
  // the instruction — additive only; a sentence with no detected list renders
  // byte-identical to before this existed.
  const notes = buildInclusionNotes(unit.tetherAnchor?.inclusionLists);
  if (sentence && notes.length > 0) {
    sentence = [sentence, ...notes.map((n) => finalize(n))].join(" ");
  }
