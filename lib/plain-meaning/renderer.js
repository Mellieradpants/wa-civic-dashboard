...
export function renderUnit(unit, { debug = false } = {}) {
  ...
  if (sectionType.type === "repeal") {
    ...
    if (analysis) {
      ...
      // (that's what makes it a status rather than a duty); every duty
      // clause otherwise keeps the sentence's one actor, inherited the same
      // way whether it's the first duty or a later one in a chain.
      const baseActor = statusText ? null : fields.actor;
      ...
      clauses.forEach((c, i) => {
        ...
        // No actor-retry fallback here: when baseActor is null because a
        // force-less status clause was found, fields.actor is the garbage
        // AAC swallowed into the actor slot (see actorSwallowedStatus above),
        // never a usable substitute — falling back to it would render that
        // garbage as if it were a real actor.
        let rendered = renderClause(clauseFields);
        if (i === 0 && statusText) rendered = attachStatusPrefix(rendered, statusText);
        if (rendered) {
          parts.push(rendered);
