...
function translateActionPhrase(verb, obj, lang) {
  ...
  if (obj) {
    ...
    let fullTrans = DICTIONARY.verbs?.[fullPhrase]?.[lang] ?? null;
    if (!fullTrans) {
      const firstObjWord = objLower.split(/\s+/)[0];
      if (firstObjWord && firstObjWord !== objLower) {
        fullTrans = DICTIONARY.verbs?.[`${verb} ${firstObjWord}`]?.[lang] ?? null;
      }
    }
    if (fullTrans) {
      return { translated: fullTrans, verbMissing: false, objMissing: false };
    }
  }

  let verbTrans = DICTIONARY.verbs?.[verb]?.[lang] ?? null;
  ...
  if (!verbTrans) {
    ...
    return { translated: null, verbMissing: true, objMissing: false };
  }

  // When obj is a comma-separated list of additional verb phrases
  // (e.g. "sell, offer for sale, distribute for sale, distribute for use in this state…"),
  // translate each token independently and rejoin. Flag only the tokens that miss.
  if (obj && obj.includes(",")) {
    return translateVerbList(verbTrans, obj, lang);
  }
  ...
  const parts = frame
    .map(s => (s === "verb" ? verbTrans : s === "object" ? (objTrans ?? obj ?? null) : null))
    .filter(Boolean);

  return { translated: parts.join(" ").trim() || null, verbMissing: false, objMissing };
}
...
function translateVerbList(primaryVerbTrans, obj, lang) {
  const items = obj.split(/,\s*/);
  const translated = [primaryVerbTrans];
  let anyMissing = false;

  for (const raw of items) {
    // Strip leading conjunctions so "or distribute for sale" → "distribute for sale"
    ...
    if (!trans) {
      ...
      if (vTrans) {
        ...
        if (o && !oTrans) anyMissing = true;
        trans = oTrans ? `${vTrans} ${oTrans}` : o ? `${vTrans} ${o}` : vTrans;
      }
    }
...
// Strips parse markers and particle notation that must never appear as literal
// output text. Applied to all Korean sentences after template fill.
...
function renderLocalizedSentence(lens, modal, fields, lang) {
  ...
  if (rawAction && lang) {
    ...
    // into a single verb key before the first-space split creates a "be" ghost.
    // e.g. "be paid" → verb="be paid", obj=null
    //      "be advanced upon request" → verb="be advanced", obj="upon request"
    const passiveMatch = /^be\s+(\w+(?:ed|en|[dt]|n))\b\s*(.*)?$/i.exec(rawAction);

    let verb, obj;
    if (passiveMatch) {
      verb = `be ${passiveMatch[1].toLowerCase()}`;
      obj = passiveMatch[2]?.trim() || null;
    } else {
      ...
      const verbRaw = spaceIdx < 0 ? rawAction : rawAction.slice(0, spaceIdx);
      verb = verbRaw.toLowerCase().replace(/[.,;:!?]+$/, "");
      obj = spaceIdx < 0 ? null : rawAction.slice(spaceIdx + 1).trim() || null;
    }
    ...
    if (result.translated) {
      ...
      // Verb itself not translatable — English fallback is cleaner than a
      // foreign-language frame wrapping an English verb.
      missingTokens = { verb, object: obj, raw: rawAction };
...
export function renderUnit(unit, lang) {
  ...
  if (lang === "ko" && sentence) {
    sentence = sanitizeKoreanOutput(sentence);
  }
