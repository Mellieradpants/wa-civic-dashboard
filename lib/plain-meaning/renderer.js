...
const CONNECTIVE_ENTRIES = Object.entries(DICTIONARY.connectives || {})
  .sort((a, b) => b[0].length - a[0].length);

const SEMANTIC_ALIASES = JSON.parse(
  readFileSync(path.join(__dirname, "..", "semantic-aliases.json"), "utf8")
);
const ALIAS_ENTRIES = (SEMANTIC_ALIASES.aliases || [])
  .map(a => [a.term, a.translations])
  .sort((a, b) => b[0].length - a[0].length);
...
function substituteAliases(text, lang) {
  if (!text || !lang || ALIAS_ENTRIES.length === 0) return text;
  let result = text;
  for (const [term, translations] of ALIAS_ENTRIES) {
...
function substituteConnectives(text, lang) {
  if (!text || !lang || CONNECTIVE_ENTRIES.length === 0) return text;
  let result = text;
  for (const [phrase, translations] of CONNECTIVE_ENTRIES) {
    const trans = translations[lang];
    if (!trans) continue;
    result = result.replace(
...
function renderLocalizedSentence(lens, modal, fields, lang) {
  ...
  if (rawAction && lang) {
    ...
    // Dictionary lookup runs on the original English verb/object — substitution
    // happens only afterward, and only on English remnants that fall back
    // untranslated, so the dictionary always sees a clean English key.
    ...
    if (result.translated) {
      ...
      if (frame[0] === "verb") {
        ...
        let objOut;
        if (objTrans) {
          objOut = lcFirst(objTrans);
        } else {
          // Object stays English — localize connective/alias phrases within it
          // (e.g. "in accordance with" -> target-language equivalent) so the
          // English-fallback remnant isn't fully untranslated.
          let fallbackObj = obj;
          if (fallbackObj) fallbackObj = substituteAliases(fallbackObj, lang);
          if (fallbackObj) fallbackObj = substituteConnectives(fallbackObj, lang);
          objOut = fallbackObj ?? null;
        }
        action = objOut ? `${verb} ${objOut}` : verb;
        // execution continues to subKey / template fill below
      } else {
        const englishRaw = TEMPLATES[lens]?.({ ...fields, enforcement: null }) ?? null;
  ...
  if (modal === "cannot" && (lang === "uk" || lang === "ru") && rawAction) {
    ...
    if (!actor) return { sentence: null, missingTokens };
    return {
      sentence: finalize(fillTemplate(tmpl, { actor, action })),
      missingTokens,
    };
  }
  ...
  if ((action == null && tmpl.includes("{action}")) ||
      (actor == null && tmpl.includes("{actor}"))) {
    return { sentence: null, missingTokens };
  }
  ...
  if (sectionType.type === "repeal") {
    ...
    if (split) {
      ...
      const renderClause = (f) => {
        if (lang && lang !== "en") {
          const result = renderLocalizedSentence(lens, f.modal, f, lang);
          if (result.missingTokens) {
            return { sentence: finalize(TEMPLATES[lens]?.(f)) || null, missingTokens: result.missingTokens };
          }
          return result;
        }
      ...
      // Some lens/language template combinations (actor_power_shift, obligation_removal,
      // the cannot+ru/uk frame) have no actor-less variant and render null without one —
      // restoring the actor is the only way to avoid silently dropping the clause.
      let r2 = renderClause(fields2);
      if (!r2.sentence && fields.actor) {
        r2 = renderClause({ ...fields2, actor: fields.actor });
      }
      ...
      sentence = result.missingTokens
        ? finalize(TEMPLATES[lens]?.(fields)) || null
        : result.sentence;
