...
function renderLocalized(lens, lang, fields) {
  ...
  const modalTr = t.modals[modal]?.[lang] || modal;
  const cond = firstCondition(conditions);
  const dl = firstDeadline(deadlines);
  ...
  if (lens === "repeal") {
    ...
    const tmpl = sub && act
      ? t.scope_change?.with_actor?.[lang]
      : t.scope_change?.no_actor?.[lang];
    if (tmpl) {
      raw = fillTemplate(tmpl, { actor: sub || "This requirement", modal: modalTr, action: act || "" });
      if (cond && !alreadyPresent(raw, cond)) raw += `, ${cond}`;
    ...
    if (hasRounding && hasCentAmount) {
      ...
      const tmpl = sub
        ? t.threshold_shift?.with_actor?.[lang]
        : t.threshold_shift?.no_actor?.[lang];
      if (tmpl) {
        raw = fillTemplate(tmpl, { actor: sub || "", modal: modalTr, action: act || "" });
        if (threshold && !alreadyPresent(raw, threshold)) raw += `, ${threshold}`;
      }
    }
  } else if (lens === "action_domain_shift") {
    const tmpl = sub
      ? t.action_domain_shift?.with_actor?.[lang]
    ...
    let tmpl;
    if (sub) {
      tmpl = t.modality_shift?.with_actor?.[lang];
    } else {
      const verb = modal === "must" ? "requires" : modal === "may" ? "allows" : "prohibits";
      tmpl = t.modality_shift?.[verb]?.[lang];
    }
    if (tmpl) {
      raw = fillTemplate(tmpl, { actor: sub || "", modal: modalTr, action: act || "follow this requirement" });
      if (cond && !alreadyPresent(raw, cond)) raw += `, ${cond}`;
      if (dl && !alreadyPresent(raw, dl)) raw += `, ${dl}`;
      if (enforcement) {
