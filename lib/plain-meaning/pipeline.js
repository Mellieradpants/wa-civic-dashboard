...
function extractSignalSentences(tracked, sentenceLineage, chain, parentNodeId, originalText, listBindings = null) {
  ...
  for (let idx = 0; idx < pieces.length; idx++) {
    ...
    const hasSignal = signals.primary !== null || signals.additional.length > 0 || isListLeadIn || Boolean(listBinding);
...
const EVALUATIVE_VALUE_RE =
  /\b(good\s+faith|fair\s+dealing|fairness|fair|equitable|equity|unreasonable|reasonable|undue|hardship|good\s+cause|unconscionable|arbitrary|capricious|best\s+interests?|public\s+interest)\b/i;
...
function buildUnit(sentenceObj, sectionType = { type: "standard" }, billId = null, chain = null) {
  ...
  // Value-term resolution detection (Cases 3 and 4) — detection only, runs
  // on the same normalized text and never feeds back into AAC extraction
  // the way inclusion-list stripping does, so there is no strip/rollback
  // step here. Case 4 runs after Case 3 and is given Case 3's own findings
  // so it never double-flags a term Case 3 already resolved.
  const delegatedValues = detectDelegatedValues(norm);
  const openValues = detectOpenValues(norm, delegatedValues);
  const valueResolution = [...delegatedValues, ...openValues];
