...
// [start, end] only reproduces this record's text by direct slicing when the
// surviving offsets are one unbroken run — true when a step only trimmed the
// edges, false when it removed something from the interior (e.g. collapsing
// a mid-text whitespace run). locateFailed flags the latter case honestly
// instead of asserting a position that can't actually be sliced back out.
function trackedSpan(tracked) {
  ...
  return {
    position: [offsets[0], offsets[offsets.length - 1] + 1],
    locateFailed: !contiguous,
  };
}
...
// Same contiguity problem as trackedSpan: the [start, end) range only slices
// back to the original text cleanly when tracked.offsets is unbroken across
// it. An earlier whitespace collapse or markup strip inside this sentence's
// span breaks that, so locateFailed is derived here rather than assumed false.
function absolutePosition(tracked, start, end) {
  ...
  for (let i = start + 1; i < end; i++) {
    if (tracked.offsets[i] !== tracked.offsets[i - 1] + 1) {
      contiguous = false;
      break;
    }
  }
  return {
    position: [tracked.offsets[start], tracked.offsets[end - 1] + 1],
    locateFailed: !contiguous,
  };
}
...
function extractSignalSentences(tracked, sentenceLineage, chain, parentNodeId) {
  ...
  for (const s of pieces) {
    ...
    const located = loc ? absolutePosition(tracked, loc.start, loc.end) : null;
