...
const MISSING_TOKENS_FILE = path.join(__dirname, "..", "missing-tokens.txt");
...
function logIfMissing(action_data, rawAction, billId) {
  ...
  const line = `${new Date().toISOString()} | ${rawAction} | ${verb} | ${obj ?? ""} | ${billId ?? ""}\n`;
  appendFile(MISSING_TOKENS_FILE, line).catch(() => {});
}
