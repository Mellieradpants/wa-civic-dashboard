...
export function runPipeline(rawText, context = {}) {
  ...
  // Strip subsection navigation markers — (1), (2)(a), (b)(i), etc. — only at
  // line start. Mid-sentence occurrences like "subject to (2) of this subsection"
  // are cross-references that carry meaning and must not be stripped.
  text = text
    .replace(/(^|\n)\s*(?:\(\d{1,2}\)|\([a-z]\))+\s*/g, "$1")
    .replace(/\s{2,}/g, " ")
    .trim();
