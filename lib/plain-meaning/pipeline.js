...
const MODAL_RE =
  /\b(is no longer required to|are no longer required to|is responsible for|are responsible for|are each repealed|is repealed|is(?:\s+hereby)?\s+appropriated|shall not|must not|may not|is obligated to|are obligated to|obligated to|shall|must|may|cannot|is required to|are required to)\b/i;
...
// ─── Action tokenizer + missing-token logger ─────────────────────────────────

const ACTION_ADVERBS = new Set([
  "knowingly", "willfully", "intentionally", "voluntarily", "unlawfully", "recklessly",
]);

function tokenizeAction(action) {
  if (!action) return null;
  let s = action.trim();
  // Strip leading adverbs so "knowingly sell" extracts "sell" not "knowingly"
  let i = s.indexOf(" ");
  while (i >= 0 && ACTION_ADVERBS.has(s.slice(0, i).toLowerCase())) {
    s = s.slice(i + 1).trim();
    i = s.indexOf(" ");
  }
  return i < 0
...
// Strip temporal phrases from an object string before dictionary lookup.
// Mirrors the subset of cleanAction that removes "within N days / no later than"
// so the check uses the same normalised form the renderer looks up.
function stripTemporalFromObj(s) {
  return s
...
// Checks whether the tokenized action is covered by the dictionary. If either
// token is absent, pushes to the Redis "missing-tokens" list for translator
// review. Uses the cleaned object form for the check to avoid false positives
// on temporal suffixes. Fire-and-forget — never blocks the pipeline on a write error.
function logIfMissing(action_data, rawAction, billId) {
  const { primary_verb: verb, direct_object: obj } = action_data;
  const cleanedObj = obj ? stripTemporalFromObj(obj) : null;

  const verbMissing = !DICTIONARY.verbs?.[verb];
  const objMissing = cleanedObj ? !DICTIONARY.objects?.[cleanedObj.toLowerCase()] : false;
  if (!verbMissing && !objMissing) return;

  if (!redis) return;
  const entry = `${new Date().toISOString()} | ${rawAction} | ${verb} | ${obj ?? ""} | ${billId ?? ""}`;
  redis.lpush("missing-tokens", entry).catch(() => {});
}

// ─── L1: 5WIH + L10: ISC — Assemble ─────────────────────────────────────────

function buildUnit(sentenceObj, sourceIndex, sectionType = { type: "standard" }, billId = null) {
  ...
  const action_data = tokenizeAction(aac.action);
  if (action_data) logIfMissing(action_data, aac.action, billId);
...
export function runPipeline(rawText, context = {}) {
  ...
  const units = signalSentences
    .map((s, i) => buildUnit(s, i, sectionType, context.billId || null))
    .filter(Boolean);
