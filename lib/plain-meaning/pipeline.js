...
export function runPipeline(rawText, context = {}) {
  ...
  // Strip "new section added" header — the boilerplate that opens every
  // addition section, e.g. "A new section is added to chapter 84.69 RCW to
  // read as follows:". Without this strip the header has no period before
  // it, so it stays glued to the first sentence; the subsection-marker strip
  // below then removes a "(1)" from the *middle* of that sentence instead of
  // its start, breaking anchorText's substring match against the raw text.
  text = text.replace(
    /^.*?\b(?:is|are\s+each)\s+added\s+to\s+(?:chapter|the\s+chapter|title)\b.*?\bto\s+read\s+as\s+follows\s*:\s*/i,
    ""
  );

  // Strip subsection navigation markers — (1), (2)(a), (b)(i), etc.
  // Only strip when preceded by a sentence boundary (start of text, or a
  // period). splitSentences below only ever breaks on ".!?", so a marker
  // after a semicolon or colon is not actually at a sentence's edge — it
  // sits mid-sentence inside a colon/semicolon-joined list, and stripping
  // it from there breaks anchorText's substring match against raw text
  // (same failure mode as the addition-header strip above). Inline
  // cross-references like "subject to (2) of this subsection" follow
  // prepositions and must be preserved.
  //
  // A marker stripped right after a period leaves SUBSECTION_BREAK behind —
  // that spot is a real sentence boundary even when the next word starts
  // with a digit, $, or lowercase letter (e.g. "(1) 50 percent...",
  // "(2) $500 must..."), which splitSentences would otherwise miss. No
  // marker precedes the start-of-text case, so nothing to mark there.
  text = text
    .replace(/(\.\s+)(?:\(\d{1,2}\)|\([a-z]\))+\s*/g, `$1${SUBSECTION_BREAK}`)
    .replace(/^(?:\(\d{1,2}\)|\([a-z]\))+\s*/, "")
    .replace(/\s{2,}/g, " ")
    .trim();
