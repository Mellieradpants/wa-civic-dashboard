...
function stripGlobal(tracked, regex) {
  ...
  for (const m of matches) {
    ...
    // Indexed loop, not offsets.push(...slice) — spreading a large array into
    // push() passes every element as its own function argument, which blows
    // the call stack on a large bill's offset array (~686,000 elements on
    // HB 1216). Same elements, same order, no behavior change.
    {
      const seg = tracked.offsets.slice(cursor, m.index);
      for (let i = 0; i < seg.length; i++) offsets.push(seg[i]);
    }
    cursor = m.index + m[0].length;
  }
  text += tracked.text.slice(cursor);
  {
    const seg = tracked.offsets.slice(cursor);
    for (let i = 0; i < seg.length; i++) offsets.push(seg[i]);
  }
...
function stripMidSubsectionMarkers(tracked, regex) {
  ...
  for (const m of matches) {
    text += tracked.text.slice(cursor, m.index);
    // Indexed loop, not offsets.push(...slice) — see stripGlobal above for
    // why: spreading a large array into push() blows the call stack on a
    // large bill's offset array. Same elements, same order, no behavior
    // change. The single-element push two lines down is not a spread and is
    // left as-is.
    {
      const seg = tracked.offsets.slice(cursor, m.index);
      for (let i = 0; i < seg.length; i++) offsets.push(seg[i]);
    ...
    text += keep;
    {
      const seg = tracked.offsets.slice(m.index, m.index + keep.length);
      for (let i = 0; i < seg.length; i++) offsets.push(seg[i]);
    }
...
// A sentence-initial subordinate clause ("If X finds that Y, MAIN") is
// closed by the comma that ends it in legal drafting — the main clause and
// its own modal begin right after. Allows an optional short subsection
// marker before the subordinator ("(6) If the director determines...", the
// real shape in HB 1311). Returns -1 when the sentence isn't fronted this
// way, so callers can leave the search window untouched.
const FRONTED_SUBORDINATOR_RE = /^\s*(?:\([^)]{1,6}\)\s*)?(If|When|Whenever|Unless|Where|Although|Though|While|After|Before|Until|Upon|Provided)\b/i;

// The end of a sentence-initial subordinate clause: the first comma at
// bracket depth 0. Depth tracking means a comma inside parentheses (e.g.
// "subsection (1)(a) and (b), of this section") doesn't falsely close the
// clause early. Returns -1 when the sentence isn't fronted that way.
function frontedClauseEndIndex(text) {
  if (!FRONTED_SUBORDINATOR_RE.test(text)) return -1;
  let depth = 0;
...
function findClauseSpan(text, markerIndex, markerLength, afterMarker) {
  ...
  let searchWindow = nextMarkerMatch ? afterMarker.slice(0, nextMarkerMatch.index) : afterMarker;

  // A marker inside a sentence-initial subordinate clause ("If X finds that
  // Y, MAIN") cannot extend past the comma that closes that clause — the
  // main clause and its modal begin there. Only ever shrinks the window: if
  // the marker sits after the comma (already in the main clause),
  // frontedEnd is not greater than markerIndex and nothing changes.
  const frontedEnd = frontedClauseEndIndex(text);
  if (frontedEnd > markerIndex) {
    const cap = frontedEnd - (markerIndex + markerLength);
    if (cap >= 0 && cap < searchWindow.length) searchWindow = searchWindow.slice(0, cap);
  }
...
//
// listBindings, when supplied, comes from detectListBindings run on this
// section's PROVISIONAL pieces — split the same way, but before the
// subsection-marker strip removes the very markers a later item needs to be
// recognized as a list continuation (see runPipeline). It's applied here by
// index position rather than recomputed on these final pieces, which no
// longer carry any marker text at all for a period-separated continuation.
// Index alignment holds because subsection-marker stripping only removes
// marker characters — it never adds or removes a sentence-ending period, so
// splitSentences produces the same number of pieces in the same order
// before and after; the length check below is a fail-safe for the case that
// assumption is somehow wrong, not the expected path.
function extractSignalSentences(tracked, sentenceLineage, chain, parentNodeId, originalText, listBindings = null) {
  const pieces = splitSentences(tracked.text);
  const bindings = listBindings && listBindings.length === pieces.length
    ? listBindings
    : new Array(pieces.length).fill(null);
  const out = [];
  let cursor = 0;
  for (let idx = 0; idx < pieces.length; idx++) {
    ...
    const listBinding = bindings[idx];
    // A detected list lead-in survives even when neither it nor any of its
    // items carries a requirement word — the fail-safe this project already
    // uses elsewhere: preserving the sentence (even in an unassigned form,
    // see buildUnit) always beats silently losing it. A bound continuation
    // survives by inheriting the lead-in's requirement word rather than
    // being dropped for lacking one of its own.
    const isListLeadIn = Boolean(listLeadInFirstItemText(s));
    // A quoted-term definition ('"X" means ...') survives the same way a
    // list lead-in does, for the same reason: Case 1 (value-term internal
    // definitions) fires on exactly this shape, but a definition sentence
    // usually carries no shall/must/may of its own, so without this it
    // would never reach buildUnit for Case 1 to run on at all. This is only
    // the cheap "does it deserve a chance" check — detectInternalDefinitions
    // in buildUnit does the real, guarded detection on normalized text.
    const isValueDefinition = INTERNAL_DEFINITION_RE.test(s);
    const hasSignal = signals.primary !== null || signals.additional.length > 0 || isListLeadIn || Boolean(listBinding) || isValueDefinition;
    const rule = signals.primary
      ? SIGNAL_RULE[signals.primary]
      : signals.additional.length
      ? "signal_in_subordinate_clause_only"
      : listBinding
      ? "list_item_bound_to_lead_in"
      : isListLeadIn
      ? "list_lead_in_no_requirement_word"
      : isValueDefinition
      ? "value_definition_no_requirement_word"
      : "no_signal_matched";
    ...
    if (hasSignal) out.push({ text: s, signals, lineage: record, listBinding });
  }
  return out;
}
...
// renderer.js's analyzeCoordinateSplit and is deliberately left alone here.
...
function detectSecondInstruction(actionText) {
  ...
  // "and which/that/who shall ..." is a relative clause continuing the previous
  // noun phrase, not a coordinate clause with a new actor. Real anchor, HB 2366:
  // "... promulgated by the state auditor and which shall substantially conform
  // to the procedures ..." — "which" refers back to "rules and regulations".
  // Splitting there emits an orphan fragment beginning "Which must ...".
  if (/^(which|that|who|whom|whose)\b/i.test(actor2)) return null;
...
// A condition clause's true end is not always its first comma. Validated
// against nine real condition sentences (H1006, H1064, H1054, H1000, H1037,
// H1074) covering "if," "unless," and "when": a comma sometimes closes the
// condition (a new independent clause begins right after — its own subject,
// its own modal) but often does not (a list continuation, a participial
// phrase, a that-clause, or the back half of a date). Naively stopping at
// every comma loses real legal content (H1054's four other acceptable bid
// deposit forms; H1000's entire "considering..., that..." substance; H1037's
// deadline year severed from its date). Naively never stopping risks the
// opposite failure this guards against elsewhere in this file (a clause
// running past the main clause and swallowing its modal) — so the boundary
// is decided comma-by-comma, not by a single greedy match.
// A subject can't be reliably recognized by a determiner whitelist — real
// bill subjects are often bare plural nouns with no article at all ("rental
// vouchers for those recipients may only be paid..."). Recognizing a
// continuation is the more reliable signal: these openers mark the fragment
// as attached to what came before (a relative/participial/complement clause
// or a citation number), never the start of a fresh main clause.
const COND_CONTINUATION_CUE_RE = /^(that|which|who|whom|whose|considering|regarding|including|excluding|notwithstanding|providing)\b/i;
// Only true modals count, not weaker copulas/auxiliaries (is/are/was/has,
// ...) — H1000's lost substance ("that there are substantial and compelling
// reasons...") contains "are", and treating that as a clause-ending signal
// would reproduce the exact defect this fix targets.
const COND_TRUE_MODAL_RE = /^(shall|must|may|cannot|will|can|could|would|should)$/i;
// Mirrors MAX_SECOND_ACTOR_WORDS's own bound on a plausible short subject
// phrase — the same class of decision ("is this a real clause boundary or
// just more of the same clause") made a few lines away in this file.
const MAX_COND_CLAUSE_SUBJECT_WORDS = 12;

function findModalWordIndex(words) {
  return words.findIndex((w) => COND_TRUE_MODAL_RE.test(w.replace(/[^a-zA-Z]/g, "")));
}

// A modal within a short opening run of words, with no continuation cue at
// the very start — the proxy for "this reads as its own complete clause"
// (same spirit as isListItemIndependentClause's subject+verb proxy, adapted
// here since a determiner whitelist misses bare-plural subjects). H1006:
// "the service contract holder ... may apply..." — modal within bound,
// condition ends here. H1000: "that there are substantial..." — starts with
// a continuation cue, never even reaches the modal check.
function isIndependentClauseStart(fragment) {
  const t = fragment.trim();
  if (!t || COND_CONTINUATION_CUE_RE.test(t) || /^\d/.test(t)) return false;
  const window = t.split(/[,;.]/)[0];
  const modalIdx = findModalWordIndex(window.split(/\s+/));
  return modalIdx !== -1 && modalIdx <= MAX_COND_CLAUSE_SUBJECT_WORDS;
}

// "..., and shall refund ..." — a second, elided-subject duty coordinated
// onto an earlier one, the same shape detectSecondInstruction and
// renderer.js's analyzeCoordinateSplit both key off elsewhere in this
// codebase. Checked separately from isIndependentClauseStart because "and"
// is not a continuation cue on its own; deliberately does not fire on "or",
// since Group B's list alternatives ("...bond, postal money order, ... or
// certified check...", "...at the time of sale, or within a longer time
// period...") are genuine continuations of the same condition, not a second
// duty.
function isCoordinateClauseStart(fragment) {
  const t = fragment.trim();
  if (!/^and\b/i.test(t)) return false;
  const window = t.split(/[,;.]/)[0];
  return findModalWordIndex(window.split(/\s+/)) !== -1;
}

// Semicolons and periods always end a condition, unchanged from before this
// fix. A comma is different: it ends the condition only when what follows is
// a genuine independent clause, a coordinated second duty, or a ", in which
// case ..." follow-on rule (see IN_WHICH_CASE_RE below) — otherwise scanning
// continues past it. This is what lets H1037's "prior to July 1, 2008" keep
// its year: "2008" starts with a digit, not a modal-bearing clause or "and",
// so the comma is not a boundary and scanning continues to the sentence's
// own period.
const IN_WHICH_CASE_RE = /^\s*in which case\b/i;

function findConditionEnd(text, startIdx) {
  let searchFrom = startIdx;
  for (;;) {
    const rest = text.slice(searchFrom);
    const m = rest.match(/[,;.]/);
    if (!m) return text.length;
    const boundaryIdx = searchFrom + m.index;
    if (text[boundaryIdx] !== ",") return boundaryIdx; // semicolon or period: hard stop
    const after = text.slice(boundaryIdx + 1);
    if (IN_WHICH_CASE_RE.test(after)) return boundaryIdx;
    if (isIndependentClauseStart(after) || isCoordinateClauseStart(after)) return boundaryIdx;
    searchFrom = boundaryIdx + 1; // not a boundary — keep scanning past this comma
  }
}

// "provided that" has zero confirmed occurrences across the corpus behind
// this fix — kept only because it was already here, not because a real
// anchor justifies it. Order does not matter: each keyword is searched for
// independently and every non-duplicate match is kept.
const CONDITION_KEYWORD_RES = [
  /\bif\b/gi,
  /\bwhen\b/gi,
  /\bunless\b/gi,
  /\bexcept\b/gi,
...
function extractConditions(norm) {
  const conditions = [];
  for (const re of CONDITION_KEYWORD_RES) {
    re.lastIndex = 0;
    let m;
    while ((m = re.exec(norm)) !== null) {
      const afterKeyword = m.index + m[0].length;
      const end = findConditionEnd(norm, afterKeyword);
      if (norm.slice(afterKeyword, end).trim().length < 4) continue;
      const c = norm.slice(m.index, end).trim();
...
// same-actor coordinate-split case in renderer.js's analyzeCoordinateSplit).
function parseActorActionCondition(text) {
  const norm = normalize(text);
  const modalMatch = norm.match(MODAL_RE);

  const conditions = extractConditions(norm);
...
// ─── List binding: colon-lettered/numbered enumerations ────────────────────
// A colon is not a sentence boundary for splitSentences, so the item right
// after it stays glued to the lead-in and survives with the lead-in — but
// every later item, once separated from the lead-in by a period, becomes its
// own standalone sentence, and if it carries no requirement word of its own
// (shall/must/may/will), L2 SSE below drops it before it can ever become a
// unit. This has been confirmed, repeatedly, against real pipeline output on
// real bills — see tmp-scan-colon-lettered-duties-r2.js and
// tmp-measure-colon-list-item-drops-results.txt, both on main.
//
// The gate below — a colon introduces a list only when the lead-in plus its
// first item reads as one continuous sentence, i.e. the first item is not
// itself independently a complete sentence with its own subject and finite
// verb — is reused unmodified from that validated discovery work, not
// re-derived. isListItemIndependentClause is a disclosed, necessarily
// approximate proxy: a subject-like opening word plus a modal, auxiliary, or
// copula verb (shall/must/may/will/can/could/would/should/is/are/was/were/
// has/have/had) appearing anywhere in the item's text. A verb of this kind
// sitting inside a relative clause ("...in which controlled substances were
// sold...") is indistinguishable, by this heuristic, from the item's own main
// verb, and can cause a genuine list item to be misread as an independent
// sentence — a known, carried-over limitation, not something this change
// attempts to resolve.
const LIST_ITEM_SUBJECT_RE = /^(The|A|An|This|That|These|Those|Any|Each|Every|No|Such|Both|Neither|Either|All|Some|Many|Most|He|She|It|They|We|You|I)\b/;
const LIST_ITEM_FINITE_AUX_RE = /\b(shall|must|may|will|can|could|would|should|is|are|was|were|has|have|had)\b/;

function isListItemIndependentClause(text) {
  const t = (text || "").trim();
  if (!LIST_ITEM_SUBJECT_RE.test(t)) return false;
  return LIST_ITEM_FINITE_AUX_RE.test(t);
}

// A marker at the very start of a piece — the shape every continuation item
// after the first takes once splitSentences has cut it away from its
// lead-in.
const LIST_MARKER_START_RE = /^\(([a-z]{1,3}|[ivxlcdm]{1,5}|\d{1,3})\)\s*/i;

// A colon immediately (within a few characters) followed by a marker — the
// shape a list's lead-in takes. Returns the first item's own text — every­
// thing from the marker to the end of the piece, since splitSentences has
// already bounded this piece at its own sentence-ending punctuation — or
// null when the colon isn't followed by a marker at all, or the "first item"
// turns out to be independently a complete sentence (Defect 2: ordinary
// statutory subsection numbering, not a real list — e.g. "...read as
// follows: (1) The superior court ... shall have jurisdiction ...").
const LIST_COLON_MARKER_RE = /:\s{0,3}\(([a-z]{1,3}|[ivxlcdm]{1,5}|\d{1,3})\)/;

function listLeadInFirstItemText(piece) {
  const m = LIST_COLON_MARKER_RE.exec(piece);
  if (!m) return null;
  const itemText = piece.slice(m.index + m[0].length).replace(/[.;]+\s*$/, "").trim();
  if (!itemText || isListItemIndependentClause(itemText)) return null;
  return itemText;
}

// One pass over a section's provisional sentence pieces — split the same way
// splitSentences always does, but BEFORE the subsection-marker strip below
// removes the very markers ("(b)", "(c)"...) this pass looks for. Returns,
// per piece index, either null (not a bound continuation) or the { actor,
// modal } inherited from the list's lead-in. The lead-in piece itself is
// never given a binding — it already produces its own unit through the
// unmodified existing path (this is the same low-error path that already
// keeps a semicolon-joined, single-piece list's items together today,
// whenever the lead-in itself carries a requirement word); only a later,
// period-separated item with no requirement word of its own needs one.
function detectListBindings(pieces) {
  const bindings = new Array(pieces.length).fill(null);
  let active = null; // { actor, modal } | null — the list currently "open"

  for (let i = 0; i < pieces.length; i++) {
    const piece = pieces[i];
    const markerStart = LIST_MARKER_START_RE.exec(piece);

    if (active && markerStart) {
      const incomingIsNumeric = /^\d+$/.test(markerStart[1]);
      if (incomingIsNumeric && active.markerType === "non-numeric") {
        // WA bills nest outline levels (1)(2)(3) > (a)(b)(c) > (i)(ii)(iii),
        // so a numbered marker appearing after a lettered/roman list is
        // always a new subsection, never a continuation of that list — a
        // structural fact about the document's own outline, not a
        // verb-based guess. Checked before isListItemIndependentClause
        // below (which is a proxy for the same "is this actually a
        // continuation" question, just from wording rather than structure)
        // since the outline-level mismatch is the more certain signal.
        active = null;
      } else {
        const itemText = piece.slice(markerStart[0].length).replace(/[.;]+\s*$/, "").trim();
        if (isListItemIndependentClause(itemText)) {
          active = null; // independent sentence — list ends here (Defect 2 shape)
        } else {
          bindings[i] = active; // genuine continuation — inherit the lead-in
        }
      }
    } else if (!markerStart) {
      active = null; // a piece with no marker at all ends the active list
    }

    // Does this piece itself establish a (possibly nested) new list? Checked
    // after the continuation check above so a piece that both continues an
    // outer list AND introduces its own nested list updates `active` to the
    // nested list for whatever follows, without disturbing the binding this
    // piece itself just received.
    const firstItemText = listLeadInFirstItemText(piece);
    if (firstItemText) {
      const aac = parseActorActionCondition(normalize(piece))[0];
      // Marker type of the list this lead-in is about to open — re-derived
      // from the same colon+marker match listLeadInFirstItemText already
      // used, so a later numbered marker can be told apart from a
      // continuation of THIS list. "i" is ambiguous between lettered and
      // roman, but that ambiguity is irrelevant here: only numeric vs.
      // non-numeric is being distinguished, which "i" is unambiguously not.
      const leadMarkerMatch = LIST_COLON_MARKER_RE.exec(piece);
      const markerType = leadMarkerMatch && /^\d+$/.test(leadMarkerMatch[1]) ? "numeric" : "non-numeric";
      active = aac.modal ? { actor: aac.actor, modal: aac.modal, markerType } : null;
    }
  }
  return bindings;
}
...
// A marker word immediately preceded by a modal (optionally "not"/"be") is
// the sentence's own verb phrase — "shall be including such items" means
// "must include such items," not a list. This isn't a real marker candidate
// at all: classifyInclusionMarker returns null and detectInclusionLists
// skips it without recording any entry, same as if the word weren't there.
const INCLUSION_MODAL_VERB_RE = /\b(?:shall|must|may|will)(?:\s+not)?(?:\s+be)?\s*$/i;
...
// Returns null when the marker candidate is actually the sentence's own verb
// phrase (see INCLUSION_MODAL_VERB_RE), not a marker at all.
function classifyInclusionMarker(sentence, markerStart, markerWord, billNumber) {
  ...
  // Gate 0 — verb-phrase guard: "shall/must/may/will [not] [be] including/
  // excluding" is the sentence's obligation verb, not a list marker.
  const beforeRaw = sentence.slice(0, markerStart);
  if (INCLUSION_MODAL_VERB_RE.test(beforeRaw)) {
    return null;
  }
...
function detectInclusionLists(sentence, billNumber) {
  ...
  while (searchFrom <= sentence.length) {
    ...
    if (entry === null) {
      // Gate 0: this occurrence is the sentence's own verb, not a marker —
      // skip it silently and keep scanning past it.
      searchFrom = markerEnd;
      continue;
    }
    results.push(entry);
    searchFrom = Math.max(entry.end, markerEnd);
  }
  return results;
}
...
// Fail-open invariant: extracting a detected list must never reduce the unit
// count. A nested "including:" inside a lettered sub-item (e.g. bill 1433's
// "(a) Establishing requirements..., including:\n(i) ...\n(ii) ...") can
// legitimately consume real obligation text along with its own lettered
// items — there is no shape rule that safely tells the two apart in
// general, so the check is on the RESULT: if stripping would leave any
// resulting unit's action empty or effectively empty (nothing left but
// whitespace, punctuation, or bare connective words), the strip is rolled
// back for the whole sentence — every unit derived from it keeps its
// original, unstripped action text. Checked here, at the one point the
// action text is actually derived, so it protects every current and future
// list shape, not just nested colon sublists.
const INCLUSION_CONNECTIVE_WORDS = new Set([
...
function isEffectivelyEmptyAction(action) {
  ...
  return words.every((w) => INCLUSION_CONNECTIVE_WORDS.has(w.toLowerCase()));
}

// ─── Value-term delegation detection (value-term / applicability-modifiers ──
// ─── layer, Case 3: delegated) ──────────────────────────────────────────────
// Detection only, run on the same L4-normalized text the inclusion/exclusion
// detection above uses. Does not touch AAC extraction, the renderer, or any
// existing field — this cluster is additive. It establishes valueResolution,
// the shared field later value-term cases (external reference, bare/open,
// internal) emit into with the same shape:
//   { valueTerm, resolutionState, resolutionHolder, cue }
// This node only ever emits resolutionState "delegated" — a value term whose
// meaning the bill hands to a named authority's judgment or rulemaking,
// rather than fixing it itself (internal), pointing to an external citation
// (external reference), or leaving it wholly unresolved (bare/open).

// Evaluative value terms — contested/normative words a bill can leave for
// someone to judge. Multi-word phrases are listed before any shorter word
// that could otherwise match a piece of them (e.g. "good faith" before a
// bare "good", "best interests" before a bare "interest") so the longer
// phrase always wins. "Unfair" and "competent" added for Case 1 (internal
// definitions), each backed by a real bill: ESHB 1233 defines "unfair
// competition," ESB 5721 defines "competent" — both require judgment the
// same way "reasonable" does, not a fixed, checkable fact.
const EVALUATIVE_VALUE_RE =
  /\b(good\s+faith|fair\s+dealing|fairness|unfair|fair|equitable|equity|unreasonable|reasonable|undue|hardship|good\s+cause|unconscionable|arbitrary|capricious|best\s+interests?|public\s+interest|competent)\b/i;

// Term-of-art value phrases — semi-standardized, so a later case may mark
// these at lower confidence than a bare evaluative word. This node treats
// them the same as EVALUATIVE_VALUE_RE for "is this a real value term worth
// resolving" purposes; the confidence distinction belongs to a later case.
const TERM_OF_ART_VALUE_RE =
  /\b(reasonable\s+time|fair\s+market\s+value|reasonable\s+notice|reasonable\s+cost)\b/i;

// Functional adjectives — "suitable/pertinent," not a contested value.
// Measured trap in the 397-bill sample behind this spec: 213 functional vs.
// 114 evaluative duty-occurrences — without this exclusion the detector
// drowns in false flags on ordinary administrative language.
const FUNCTIONAL_VALUE_RE = /\b(appropriate|adequate|suitable|relevant|applicable)\b/i;

// A pattern this node must never treat as delegated, because the resolution
// is actually external (RCW/chapter/section/U.S.C./CFR) rather than handed
// to an authority's judgment — that is a separate, later case (external
// reference), not this one. None of the cue patterns below can textually
// overlap this phrase today (they require "deems", "determined/prescribed
// by", or "finds that" — never a bare "defined in"), so this check is an
// explicit, defensive guard rather than something the fixtures below
// currently exercise. Still used by the DETERMINED_CUE_RE loop below.
const EXTERNAL_REFERENCE_RE = /\bas\s+defined\s+(?:in|under)\s+(?:RCW|chapter|section|U\.S\.C\.|CFR)\b/i;

function cleanCapture(s) {
  return (s || "").replace(/\s+/g, " ").replace(/^[,;\s]+|[,;\s]+$/g, "").trim();
}

// A holder captured as a bare back-reference pronoun ("it", "they"...) is
// resolved to the sentence's own main actor — the same actor L5 AAC would
// extract — rather than left as a pronoun. Reuses BACK_REFERENCE_PRONOUNS,
// the same set the second-instruction detector already treats as "refers
// back," not a new actor. Only the capture's own last word is checked (not
// the whole string), since a cue's actor group can pick up a little leading
// context (e.g. "only if it") without that context changing what the actor
// itself resolves to.
function resolveDelegationHolder(rawActor, mainActor) {
  const holder = cleanCapture(rawActor);
  if (!holder) return mainActor || null;
  const lastWord = holder.split(/\s+/).pop().toLowerCase();
  if (BACK_REFERENCE_PRONOUNS.has(lastWord) && mainActor) return mainActor;
  return holder;
}

// Every "actor named right before a trigger verb" capture below is
// deliberately capped at one or two words (not a character count). A real
// actor in this position is always short — a pronoun or a short institutional
// name ("it", "the board", "the department") — and a tight word cap is what
// keeps a lazy backward capture from walking all the way back across the
// sentence's own modal and verb to satisfy the same trigger phrase further
// left (JS regex tries the leftmost successful start position, so a looser
// cap would let it).
const ACTOR_BEFORE_TRIGGER = "([a-z][\\w']*(?:\\s+[a-z][\\w']*)?)";

// Cue: "as <ACTOR> deems <VALUE>" / "as <ACTOR> may deem <VALUE>" — e.g. HB
// 1278's "...as it deems necessary...". "Deem" itself means to judge or form
// an opinion, so whatever follows it is evaluative by construction; this cue
// does not require its object to appear in EVALUATIVE_VALUE_RE (the real
// bill behind it uses "necessary," which isn't on that list) — it is gated
// only against FUNCTIONAL_VALUE_RE, since a functional adjective ("deems it
// appropriate") still isn't a contested value even inside this cue.
const DEEMS_CUE_RE = new RegExp(
  `\\bas\\s+${ACTOR_BEFORE_TRIGGER}\\s+(?:deems|may\\s+deem)\\s+([a-z][\\w-]*)`,
  "gi"
);

// Cue: "as (determined|prescribed) by <ACTOR>" — e.g. HB 1263's "...as
// determined by the department...". This cue modifies a noun rather than
// introducing a value inline, and the real bill sentence behind it places
// the value term ("good cause") earlier in the sentence, as its own subject,
// not adjacent to the cue — so this cue has no inline object; the value term
// is found via findLexiconValueTerm below, scanning the whole sentence.
const DETERMINED_CUE_RE =
  /\bas\s+(?:determined|prescribed)\s+by\s+([a-z][\w' ]{0,50}?)(?=[,;.]|\s+(?:when|that|and|or|unless)\b|$)/gi;

// Cue: "if <ACTOR> finds that <VALUE> exists" — e.g. HB 2046's "...only if
// it finds that good cause exists...". Tried first because it pins down the
// value phrase precisely (everything up to " exists"); FINDS_THAT_CUE_RE
// below is the fallback for the "exists"-less variant ("<ACTOR> finds that
// <VALUE>"). Gated against the lexicon in detectDelegatedValues: "finds
// that" alone is also how a sentence states an ordinary fact ("finds that
// the report was filed"), so an inline object here only counts as
// delegation when it is actually a value word, not just because the verb is
// "finds."
const FINDS_THAT_EXISTS_CUE_RE = new RegExp(
  `\\b(?:if\\s+)?${ACTOR_BEFORE_TRIGGER}\\s+finds\\s+that\\s+([a-z][\\w' ]{0,40}?)\\s+exists?\\b`,
  "gi"
);
const FINDS_THAT_CUE_RE = new RegExp(
  `\\b(?:if\\s+)?${ACTOR_BEFORE_TRIGGER}\\s+finds\\s+that\\s+([a-z][\\w' ]{0,40}?)(?=[,;.]|$)`,
  "gi"
);

// Finds the first evaluative or term-of-art value term anywhere in the
// sentence, for cues that modify a noun rather than introduce a value
// inline (DETERMINED_CUE_RE) — the value term in a real bill sentence is not
// always adjacent to the delegation phrase (HB 1263 states "good cause" as
// the sentence's own subject, well before "as determined by the
// department").
function findLexiconValueTerm(sentence) {
  const m = sentence.match(EVALUATIVE_VALUE_RE) || sentence.match(TERM_OF_ART_VALUE_RE);
  return m ? cleanCapture(m[0]) : null;
}

function detectDelegatedValues(sentence) {
  const findings = [];
  const mainActor = parseActorActionCondition(sentence)[0]?.actor || null;

  for (const m of sentence.matchAll(DEEMS_CUE_RE)) {
    const value = cleanCapture(m[2]);
    if (!value || FUNCTIONAL_VALUE_RE.test(value)) continue;
    findings.push({
      valueTerm: value,
      resolutionState: "delegated",
      resolutionHolder: resolveDelegationHolder(m[1], mainActor),
      cue: cleanCapture(m[0]),
    });
  }

  for (const m of sentence.matchAll(DETERMINED_CUE_RE)) {
    if (EXTERNAL_REFERENCE_RE.test(m[0])) continue;
    const value = findLexiconValueTerm(sentence);
    if (!value) continue;
    findings.push({
      valueTerm: value,
      resolutionState: "delegated",
  ...
  // "Exists" variant tried first; the fallback only runs when it found
  // nothing, so a sentence matching the exists-variant is never also
  // double-counted by the looser fallback pattern.
  const findsThatMatches = [...sentence.matchAll(FINDS_THAT_EXISTS_CUE_RE)];
  const findsThatSource = findsThatMatches.length
    ? findsThatMatches
    : [...sentence.matchAll(FINDS_THAT_CUE_RE)];
  for (const m of findsThatSource) {
    const candidate = cleanCapture(m[2]);
    if (!candidate) continue;
    const lexiconMatch = candidate.match(EVALUATIVE_VALUE_RE) || candidate.match(TERM_OF_ART_VALUE_RE);
    if (!lexiconMatch) continue;
    findings.push({
      valueTerm: cleanCapture(lexiconMatch[0]),
      resolutionState: "delegated",
      resolutionHolder: resolveDelegationHolder(m[1], mainActor),
      cue: cleanCapture(m[0]),
    });
...
// A duty modal for Case 4's own purposes — narrower than MODAL_RE (no bare
// "may", since "may" is permission, not a duty commanding a standard the
// actor must meet). "Commanded position" is defined relative to this modal:
// the sentence span from its first occurrence onward, not the whole
// sentence — matches the proven reference behavior validated for this case.
const DUTY_MODAL_FOR_OPEN_RE = /\b(?:shall\s+not|may\s+not|shall|must)\b/i;

// Broader than EXTERNAL_REFERENCE_RE (Case 3's own guard, which only matches
// "as defined in") — real bill text (SHB 1621) resolves a value term via
// "established in RCW," not "defined in," so Case 4 needs its own, wider
// external-cue check. Additive: EXTERNAL_REFERENCE_RE itself is untouched
// and still used, unmodified, by the DETERMINED_CUE_RE loop above.
const EXTERNAL_CUE_BROAD_RE =
  /\b(?:as\s+)?(?:defined|established|provided|set\s+forth|prescribed)\s+(?:in|under)\s+(?:RCW|chapter|section|U\.S\.C\.|CFR)\b/i;

// Case 4: the residual case. Flags an evaluative or term-of-art value term
// that a duty commands but the bill never resolves — not defined, not
// pointed elsewhere, not handed to a named authority. Runs after Case 3 and
// only over what Case 3's own findings (delegatedFindings, passed in) did
// not already resolve, so a term is never flagged as both delegated and
// open.
function detectOpenValues(sentence, delegatedFindings) {
  const modalMatch = sentence.match(DUTY_MODAL_FOR_OPEN_RE);
  if (!modalMatch) return [];

  const commandedSpan = sentence.slice(modalMatch.index);
  const value = findLexiconValueTerm(commandedSpan);
  if (!value) return [];

  // Guard (a): already resolved by Case 3 — never double-flag the same term.
  const alreadyDelegated = delegatedFindings.some(
    (f) => f.valueTerm.toLowerCase() === value.toLowerCase()
  );
  if (alreadyDelegated) return [];

  // Guard (b): a functional adjective is not a contested value, even though
  // this path can't actually surface one today (EVALUATIVE_VALUE_RE and
  // TERM_OF_ART_VALUE_RE never match a functional word in the first place) —
  // kept as an explicit, defensive check rather than relying on that.
  if (FUNCTIONAL_VALUE_RE.test(value)) return [];

  // Guard (c): external reference — the term's meaning is pushed to a
  // citation, not left open. Broader than Case 3's own guard on purpose;
  // see EXTERNAL_CUE_BROAD_RE above.
  if (EXTERNAL_CUE_BROAD_RE.test(sentence)) return [];

  // Guard (d): a definitions-section entry ("<term> means ..."). Defensive —
  // a definitions entry has no duty modal, so it usually never reaches this
  // point at all; no fixture below exercises this guard, it only ever
  // suppresses.
  const definitionCueRe = new RegExp(
    `"?${value.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}"?\\s+means\\b`,
    "i"
  );
  if (definitionCueRe.test(sentence)) return [];

  return [{ valueTerm: value, resolutionState: "open", resolutionHolder: null, cue: null }];
}

// Cue: a quoted term followed by "means" plus a substantive body — e.g. SSB
// 5101's '"undue hardship" means an action requiring significant difficulty
// or expense.'. Fires on the definition sentence itself; no duty modal is
// needed (Case 1 is independent of Cases 3 and 4, which only fire inside a
// commanded duty).
const INTERNAL_DEFINITION_RE = /"([^"]{3,50})"\s+means\b(.*)/i;

// A term that merely CONTAINS a value word but is a technical/financial/
// defined-program term, not a value being operationalized — e.g. E2SHB
// 1686's "private equity fund" contains "equity" but names a type of
// company, not a contested value. Accumulating list, same convention as the
// value lexicons above: each entry needs a real bill behind it.
const TECH_COMPOUND_RE =
  /\b(private\s+equity|equity\s+fund|equity\s+plan|fair\s+market\s+value|true\s+and\s+fair\s+value|fair\s+value|labor\s+practice)\b/i;

// A definition body that is only a pointer to external law (e.g. SB 5422's
// '"unfair labor practice" means any unfair labor practice listed in RCW
// 41.80.110') is not a self-contained internal definition — it's
// external-reference-shaped, and belongs with deferred Case 2
// (data/wa/value-term-deferred-cases.md), not this one.
const EXTERNAL_POINTER_BODY_RE =
  /^\s*(?:any\s+[\w ]+?\s+)?(?:listed|defined|provided|set forth)\s+in\s+(?:RCW|chapter|section)/i;

// Case 1: the bill defines the value term itself. Independent of Cases 3
// and 4 — it fires on a definition sentence, they fire on a duty sentence,
// so they never collide on the same unit.
function detectInternalDefinitions(sentence) {
  const m = sentence.match(INTERNAL_DEFINITION_RE);
  if (!m) return [];

  const term = cleanCapture(m[1]);
  const body = m[2] || "";

  // Guard (a): the quoted term itself must be evaluative — a definition of
  // a non-value term is not Case 1.
  if (!EVALUATIVE_VALUE_RE.test(term) && !TERM_OF_ART_VALUE_RE.test(term)) return [];

  // Guard (b): technical/financial/program-term homograph.
  if (TECH_COMPOUND_RE.test(term)) return [];

  // Guard (c): the body only points to external law — that's deferred
  // Case 2's shape, not a self-contained internal definition.
  if (EXTERNAL_POINTER_BODY_RE.test(body)) return [];

  return [{ valueTerm: term, resolutionState: "internal", resolutionHolder: null, cue: "means" }];
}
...
function buildUnit(sentenceObj, sectionType = { type: "standard" }, billId = null, chain = null) {
  const { text, signals, lineage, listBinding } = sentenceObj;
  ...
  // Value-term resolution detection (Cases 1, 3, and 4) — detection only,
  // runs on the same normalized text and never feeds back into AAC
  // extraction the way inclusion-list stripping does, so there is no
  // strip/rollback step here. Case 4 runs after Case 3 and is given Case
  // 3's own findings so it never double-flags a term Case 3 already
  // resolved. Case 1 fires on definition sentences and Case 4 fires on duty
  // sentences, so they never collide on the same unit — no ordering
  // dependency between them.
  const delegatedValues = detectDelegatedValues(norm);
  const openValues = detectOpenValues(norm, delegatedValues);
  const internalValues = detectInternalDefinitions(norm);
  const valueResolution = [...delegatedValues, ...openValues, ...internalValues];
  const normForAac = stripInclusionListSpans(norm, inclusionLists);
  const strippedAacList = parseActorActionCondition(normForAac);

  // Fail-open invariant: extraction must never reduce the unit count this
  // sentence produces, and must never leave a resulting unit's action empty
  // or effectively empty. Comparing against the unstripped parse — not just
  // inspecting the stripped result on its own — is what actually catches a
  // nested list (e.g. a lettered sub-item's own "including:") consuming a
  // genuine embedded second instruction along with its own lettered items:
  // the FIRST unit's action can still look non-empty while a SECOND unit
  // the unstripped text would have produced disappears entirely, so an
  // empty-action check alone would miss it.
  const unstrippedAacList = parseActorActionCondition(norm);
  const wouldReduceUnitCount = strippedAacList.length < unstrippedAacList.length;
  const wouldLeaveEmptyAction = strippedAacList.some((aac) => isEffectivelyEmptyAction(aac.action));
  const extracted = !wouldReduceUnitCount && !wouldLeaveEmptyAction;
  const aacList = extracted ? strippedAacList : unstrippedAacList;
  const annotatedInclusionLists = inclusionLists.map((l) => ({
    ...l,
    extracted: Boolean(l.excisionSpan) && extracted,
  }));

  if (lineage && chain) {
    // Same convention as the L3 CFS record just above: this record's text
    // must match lineage.position in the ROOT text, so it carries the raw
    // sentence text (not norm, which normalize() may have already changed,
    // e.g. by stripping a "Sec. N." prefix) — detection itself still runs
    // on norm; only what's recorded here differs.
    makeRecord(chain, {
      parentNodeId: lineage.id,
      text,
      ...
      rule: inclusionLists.length
        ? inclusionLists.map((l) => l.classification).join(",") +
          (!extracted
            ? ` [fail-open: extraction skipped, ${wouldReduceUnitCount ? "would reduce unit count" : "would leave empty action"}]`
            : "")
        : "no_marker_found",
  ...
  // L5 AAC — one instruction, or two when a genuine second one is found
  ...
  return aacList.map((aac, i) => {
    ...
    // A bound list item — a period-separated continuation with no
    // requirement word of its own — has no modal from its own L5 AAC parse
    // (aac.modal is null, which is exactly why it had no signal); inherit
    // the lead-in's actor/modal instead of producing an empty unit. Scoped
    // to the first (non-split) group only: a genuine second "and"-joined
    // instruction (detectSecondInstruction) always has its own modal by
    // construction, so a split group is never a candidate for this.
    const bound = !isSplit && !aac.modal && listBinding;
    const effectiveActor = bound ? listBinding.actor : aac.actor;
    const effectiveModal = bound ? listBinding.modal : aac.modal;
    // A detected list lead-in with no requirement word anywhere — neither
    // itself nor any item — has nothing to inherit either. Falling back to
    // the sentence's own full normalized text as the action, rather than
    // leaving it null, is the same fail-safe already used elsewhere in this
    // project (list-boundary extraction, the and-split rule): preserving
    // the sentence in an unassigned form always beats losing it outright.
    const effectiveAction = (!isSplit && !aac.modal && !aac.action) ? norm : aac.action;
    const signal = isSplit
      ? signalForModal(aac.modal)
      : (signals.primary ?? (bound ? signalForModal(effectiveModal) : null));
    ...
    // renderer.js's analyzeCoordinateSplit makes for the same-actor coordinate-split case.
    const wih = {
      what: {
        claim: text,
      ...
      who: {
        responsibleParty: effectiveActor,
    ...
    return {
      ...
      tetherAnchor: {
        ...
        inclusionLists: isSplit ? [] : annotatedInclusionLists,
        // New, additive field — the shared value-term resolution slot later
        // cases (external reference, bare/open, internal) will emit into
        // with the same shape. Same split-attribution rule as
        // subordinateClauseSignals/inclusionLists: attributed only to the
        // first instruction of a split sentence, since a detected value
        // term can't be reliably assigned to one clause or the other.
        // Empty array when no value term is present, and absent entirely on
        // units from the API's `units` input path — every reader must treat
        // a missing field the same as an empty array.
        valueResolution: isSplit ? [] : valueResolution,
        traceReason,
      },
      parse: wih,
...
export function runPipeline(rawText, context = {}) {
  ...
  // List-item binding must be computed on the PROVISIONAL split — the same
  // splitSentences call extractSignalSentences will make later, but run here
  // while markers like "(b)", "(c)" are still present in the text. The
  // subsection-marker strip just below removes exactly those markers from
  // any piece that follows a period, which is what a list continuation item
  // looks like once it's been cut away from its lead-in — by the time
  // extractSignalSentences would otherwise see it, there is no marker left
  // to recognize it by at all. See detectListBindings for the binding rule
  // itself and extractSignalSentences for how it's applied by index.
  const provisionalPieces = splitSentences(tracked.text);
  const listBindings = detectListBindings(provisionalPieces);
  ...
  const signalSentences = extractSignalSentences(tracked, sentenceLineage, chain, record.id, secText, listBindings); // L2 SSE
