...
function makeNode(chain, text, producedBy, position) {
  const node = { text, producedBy, position };
  chain.nodes.push(node);
  return node;
}

function makeEdge(chain, step, rule, matched) {
  const edge = { step, rule, matched };
...
function extractSignalSentences(tracked, sentenceLineage) {
  ...
  for (const s of pieces) {
    ...
    const record = {
      node: { text: s, producedBy: "sentence_split", position },
      signalEdge: {
        step: "L2 SSE",
...
export function runPipeline(rawText, context = {}) {
  ...
  makeNode(chain, tracked.text, "section_split", trackedSpan(tracked));

  // Strip (( ... )) WA legislative markup (struck/substituted text)
  {
    const { tracked: next, matched } = stripGlobal(tracked, STRIKEOUT_RE);
    makeEdge(chain, "strikeout_strip", "(( )) markup", matched);
    tracked = next;
    makeNode(chain, tracked.text, "strikeout_strip", trackedSpan(tracked));
  }

  // Whitespace collapse + trim
  {
    const { tracked: next, matched } = collapseRunsAndTrim(tracked, 1);
    makeEdge(chain, "whitespace_normalize", "collapse + trim", matched);
    tracked = next;
    makeNode(chain, tracked.text, "whitespace_normalize", trackedSpan(tracked));
  }
  ...
  // Strip amendment header
  {
    ...
    makeEdge(chain, "amendment_header_strip", "is/are each amended to read as follows", matched);
    tracked = next;
    makeNode(chain, tracked.text, "amendment_header_strip", trackedSpan(tracked));
  }

  // Strip "new section added" header
  {
    const { tracked: next, matched } = stripPrefix(tracked, NEW_SECTION_HEADER_RE);
    makeEdge(chain, "new_section_header_strip", "is/are each added to ... read as follows", matched);
    tracked = next;
    makeNode(chain, tracked.text, "new_section_header_strip", trackedSpan(tracked));
  }
  ...
  // semicolon/colon because they aren't a real sentence boundary.
  {
    makeEdge(
      chain,
      "subsection_marker_strip",
      "marker after semicolon/colon — not a sentence boundary",
      SKIPPED_SUBSECTION_RE.test(tracked.text)
    );

    const mid = stripMidSubsectionMarkers(tracked, MID_SUBSECTION_RE);
    makeEdge(chain, "subsection_marker_strip", "marker after sentence boundary (period)", mid.matched);
    tracked = mid.tracked;

    const lead = stripPrefix(tracked, LEADING_SUBSECTION_RE);
    makeEdge(chain, "subsection_marker_strip", "marker at start of text", lead.matched);
    tracked = lead.tracked;

    const cleaned = collapseRunsAndTrim(tracked, 2);
    makeEdge(chain, "whitespace_normalize", "collapse runs of 2+ / trim", cleaned.matched);
    tracked = cleaned.tracked;

    makeNode(chain, tracked.text, "subsection_marker_strip", trackedSpan(tracked));
  }

  const sentenceLineage = [];
  const signalSentences = extractSignalSentences(tracked, sentenceLineage); // L2 SSE
