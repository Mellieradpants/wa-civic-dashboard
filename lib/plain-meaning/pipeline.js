...
// reference), not this one. None of the five cue patterns below can
// textually overlap this phrase today (they require "deems", "determined/
// prescribed by", "rules of", "finds that", or "discretion of" — never a
// bare "defined in"), so this check is an explicit, defensive guard rather
// than something the fixtures below currently exercise.
...
// Cue: "as defined by rules of <ACTOR>" — delegated rulemaking, e.g. HB
// 1433's "...as defined by rules of the disciplining authority...". Distinct
// from "as defined in RCW/chapter/..." (EXTERNAL_REFERENCE_RE) — defining a
// term BY RULEMAKING is itself the delegation, independent of whether the
// term being defined sits on the evaluative lexicon, so this cue captures
// the (short) phrase immediately before it as the value term rather than
// requiring lexicon membership.
const DEFINED_BY_RULES_CUE_RE = new RegExp(
  `\\b${ACTOR_BEFORE_TRIGGER},?\\s+as\\s+defined\\s+by\\s+rules\\s+of\\s+([a-z][\\w' ]{0,50}?)(?=[,;.]|$)`,
  "gi"
);
...
// Cue: "in the (sole )?discretion of <ACTOR>". No specific bill citation
// behind this bullet the way the other four have one, but it is common
// statutory boilerplate; included per spec. No dedicated fixture below —
// every fixture in the new suite is a real, cited bill sentence.
const DISCRETION_OF_CUE_RE = /\bin\s+the\s+(?:sole\s+)?discretion\s+of\s+([a-z][\w' ]{0,50}?)(?=[,;.]|$)/gi;
...
// inline (DETERMINED_CUE_RE, DISCRETION_OF_CUE_RE) — the value term in a
// real bill sentence is not always adjacent to the delegation phrase (HB
// 1263 states "good cause" as the sentence's own subject, well before "as
...
function detectDelegatedValues(sentence) {
  ...
  for (const m of sentence.matchAll(DEFINED_BY_RULES_CUE_RE)) {
    if (EXTERNAL_REFERENCE_RE.test(m[0])) continue;
    ...
    findings.push({
      valueTerm: value,
      resolutionState: "delegated",
      resolutionHolder: resolveDelegationHolder(m[2], mainActor),
      cue: cleanCapture(m[0]),
    });
  }
  ...
  for (const m of sentence.matchAll(DISCRETION_OF_CUE_RE)) {
    if (EXTERNAL_REFERENCE_RE.test(m[0])) continue;
    const value = findLexiconValueTerm(sentence);
    if (!value) continue;
    findings.push({
...
function buildUnit(sentenceObj, sectionType = { type: "standard" }, billId = null, chain = null) {
  ...
  // Value-term delegation detection (Case 3) — detection only, runs on the
  // same normalized text and never feeds back into AAC extraction the way
  // inclusion-list stripping does, so there is no strip/rollback step here.
  const valueResolution = detectDelegatedValues(norm);
